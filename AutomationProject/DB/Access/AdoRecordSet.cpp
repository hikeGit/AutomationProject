/*########################################################################
  ...
  ########################################################################*/
#include "stdafx.h"
#include "..\..\include\DB\AdoRecordSet.h"
#include <math.h>
#include <io.h>
...
/*========================================================================
	Params:		
		- strSQL:		SQLLL语句, 表名, 存储过程调用或持久 Recordset 文件名.
		...
		[常量]				[说明] 
		-------------------------------------------------
		adCmdText			指示strSQL为命令文本, 即普通的SQLLL语句. 
		adCmdTable			指示ADO生成SQLLL查询返回以 strSQL 命名的表中的
						所有行. 
...
CString CAdoRecordSet::GetFieldName(long lIndex)
{
	...
	try
	{
		strFieldName = LPCSTR(m_pRecordset->Fields->GetItem(_variant_t(lIndex))->GetName());
	}
	catch (_com_error e)
	{
...
BOOL CAdoRecordSet::GetCollect(LPCSTR strFieldName, COleCurrency &value)
{
	ASSERT(m_pRecordset != NULL);
...
BOOL CAdoRecordSet::GetCollect(LPCSTR strFieldName, CString &strValue)
{
	ASSERT(m_pRecordset != NULL);
	try
	{
...
BOOL CAdoRecordSet::AppendChunk(LPCSTR strFieldName, LPVOID lpData, UINT nBytes)
{
	ASSERT(m_pRecordset != NULL);
	ASSERT(lpData != NULL);
	if (adFldLong & GetFieldAttributes((long)strFieldName))
	{
		return AppendChunk(GetField((long)strFieldName), lpData, nBytes);
	}
	else return FALSE;
}
...
BOOL CAdoRecordSet::AppendChunk(LPCSTR strFieldName, LPCTSTR lpszFileName)
{
	...
	if (adFldLong & GetFieldAttributes((long)strFieldName))
	{
		...
		if (file.Open(lpszFileName, CFile::modeRead))
		{
			...
			if (pbuf != NULL && file.Read(pbuf, length) == (DWORD)length)
			{
				bret = AppendChunk(GetField((long)strFieldName), pbuf, length);
			}
			if (pbuf != NULL) delete[] pbuf;
		}
		file.Close();
...
{
	if (adFldLong & GetFieldAttributes((long)strFieldName))
		return  GetChunk(GetField((long)strFieldName), lpData);
	else return FALSE;
}
...
BOOL CAdoRecordSet::GetChunk(long index, CBitmap &bitmap)
{
	CString str;
	char *pStr;
	str=GetFieldName(index);
	pStr = (char*)str.GetBuffer();
	return GetChunk(pStr, bitmap);
}

BOOL CAdoRecordSet::GetChunk(LPCSTR strFieldName, CBitmap &bitmap)
{		
	BOOL bret = FALSE;
	long size = GetFieldActualSize((long)strFieldName);
	if ((adFldLong & GetFieldAttributes((long)strFieldName)) && size > 0)
	{
		BYTE *lpData = new BYTE[size];
		
		if (GetChunk(GetField((long)strFieldName), (LPVOID)lpData))
		{
